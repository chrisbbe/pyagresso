import os
import pytest
from pyagresso.api import Agresso, AgressoRequestType
...
class TestPyAgresso(object):
    ...
    def test_constructor(self):
        ag = Agresso(username=self.username, password=self.password, client=self.client,
        ...
        with pytest.raises(ValueError) as te:
            ag = Agresso(password=self.password, client=self.client,
                         instance_url=self.instance_url)
